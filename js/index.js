...
// Initialize system as controllable device
function initializeSystemControl() {
    ...
    systemPeer.on('open', (id) => {
        console.log('System ready for remote control. ID:', id);
        // Display the ID somewhere on your system for pairing
        showSystemId(id);
    });

    systemPeer.on('connection', (conn) => {
        dashboardConnection = conn;
        setupSystemConnection();
        console.log('Dashboard connected:', conn.peer);
    });

    systemPeer.on('error', (err) => {
        console.error('System peer error:', err);
    });
}

function setupSystemConnection() {
    dashboardConnection.on('data', (data) => {
        console.log('Received command:', data);
        executeSystemCommand(data);
    });

    dashboardConnection.on('close', () => {
        console.log('Dashboard disconnected');
        dashboardConnection = null;
    });
}

function executeSystemCommand(commandData) {
    const { command, timestamp } = commandData;
    
    switch (command) {
        case 'blackout':
            blackoutScreen();
            sendResponse('Blackout mode activated');
            break;
        case 'refresh':
            window.location.reload();
            break;
        case 'mute':
            muteAllMedia();
            sendResponse('All media muted');
            break;
        case 'fullscreen':
            requestFullscreen();
            sendResponse('Fullscreen requested');
            break;
        default:
...
function sendResponse(message) {
    if (dashboardConnection && dashboardConnection.open) {
        dashboardConnection.send({
            type: 'response',
            message: message,
            timestamp: Date.now()
        });
...
function showSystemId(id) {
    // Create a small overlay to display the system ID
    const idOverlay = document.createElement('div');
    idOverlay.style.cssText = `
        position: fixed;
        top: 20px;
        right: 20px;
        ...
        backdrop-filter: blur(10px);
    `;
    idOverlay.innerHTML = `System ID: ${id}<br><small>Use this ID to connect from dashboard</small>`;
    document.body.appendChild(idOverlay);
    
    // Auto-hide after 10 seconds
    setTimeout(() => {
        if (document.body.contains(idOverlay)) {
            document.body.removeChild(idOverlay);
        }
    }, 10000);
}
