...
async function createFullscreenEmbed(url) {
    const outgoingEmbed = document.querySelector('.fullscreen-embed[style*="display: block"]');
    const isSwitching = !!outgoingEmbed;

    // If the target app is already active on screen, do nothing.
    if (isSwitching && outgoingEmbed.dataset.embedUrl === url) {
        return;
    }

    closeControls();

    // Find or create the INCOMING embed
    let incomingEmbed = document.querySelector(`.fullscreen-embed[data-embed-url="${url}"]`);
    if (!incomingEmbed) {
        // Create new embed if not already minimized
        const iframe = document.createElement('iframe');
        iframe.src = url;
        iframe.style.pointerEvents = 'none'; // Prevent interaction during animation
        iframe.setAttribute('data-gurasuraisu-iframe', 'true');
        ...
        incomingEmbed = document.createElement('div');
        incomingEmbed.className = 'fullscreen-embed';
        incomingEmbed.dataset.embedUrl = url;
        incomingEmbed.appendChild(iframe);
        document.body.appendChild(incomingEmbed);

        // Re-enable pointer events after a short delay
        setTimeout(() => { iframe.style.pointerEvents = 'auto'; }, 350);
    }
    
    // Cancel any pending cleanup timer for the app we are opening
    ...
    if (appName) {
        ...
        if (appDetails && appDetails.icon) {
            let iconUrl = appDetails.icon.startsWith('/') ? appDetails.icon : `/assets/appicon/${appDetails.icon}`;
    ...
    // --- Animation Logic ---
    if (isSwitching) {
        // --- FAST APP SWITCH ANIMATION ---
        // 1. Prepare incoming embed: make it visible but transparent
        incomingEmbed.style.transition = 'opacity 0.2s ease-in-out';
        incomingEmbed.style.opacity = '0';
        incomingEmbed.style.display = 'block';
        incomingEmbed.style.transform = 'scale(1)'; // Ensure it's full size
        incomingEmbed.style.borderRadius = '0px';

        // 2. Animate outgoing embed to transparent
        outgoingEmbed.style.transition = 'opacity 0.2s ease-in-out';
        outgoingEmbed.style.opacity = '0';
        
        // 3. Force reflow, then fade in the new embed
        void incomingEmbed.offsetWidth;
        incomingEmbed.style.opacity = '1';

        // 4. Clean up the outgoing embed after the animation
        setTimeout(() => {
            if (outgoingEmbed.dataset.embedUrl) {
                minimizedEmbeds[outgoingEmbed.dataset.embedUrl] = outgoingEmbed;
            }
            outgoingEmbed.style.display = 'none';
        }, 200); // Match transition duration

    } else {
        // --- STANDARD OPEN FROM HOME ANIMATION ---
        ...
        incomingEmbed.style.display = 'block';
        incomingEmbed.style.transform = 'scale(0.8)';
        incomingEmbed.style.opacity = '0';
        incomingEmbed.style.borderRadius = '35px';
        incomingEmbed.style.cornerShape = 'superellipse(1.5)';
        incomingEmbed.style.border = '1px solid var(--glass-border)';
        incomingEmbed.style.overflow = 'clip';
        incomingEmbed.style.transition = 'transform 0.3s ease, opacity 0.3s ease, border-radius 0.3s ease';
        ...
        setTimeout(() => {
            incomingEmbed.style.transform = 'scale(1)';
            incomingEmbed.style.opacity = '1';
            incomingEmbed.style.borderRadius = '0px';
            incomingEmbed.style.cornerShape = 'square';
            incomingEmbed.style.border = 'none';
        }, 10);
    }
    populateDock();
}
