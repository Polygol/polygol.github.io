# Open Source Licenses & Acknowledgements
It does not include our self made assets/libraries/frameworks.

## Libraries & Frameworks
*   **Moment.js**
    *   License: [MIT](https://opensource.org/licenses/MIT)
    *   Project: [https://momentjs.com/](https://momentjs.com/)
*   **html2canvas**
    *   License: [MIT](https://opensource.org/licenses/MIT)
    *   Project: [https://html2canvas.hertzen.com/](https://html2canvas.hertzen.com/)
*   **modern-screenshot**
    *   License: [MIT](https://opensource.org/licenses/MIT)
    *   Project: [https://github.com/qq15725/modern-screenshot]
*   **Color Thief**
    *   License: [MIT](https://opensource.org/licenses/MIT)
    *   Project: [https://lokeshdhakar.com/projects/color-thief/](https://lokeshdhakar.com/projects/color-thief/)
*   **Three.js**
    *   License: [MIT](https://opensource.org/licenses/MIT)
    *   Project: [https://threejs.org/](https://threejs.org/)
*   **Simplex Noise**
    *   License: [MIT](https://opensource.org/licenses/MIT)
    *   Project: [https://github.com/jwagner/simplex-noise.js](https://github.com/jwagner/simplex-noise.js)
*   **Trystero** (Waves Connection)
    *   License: [MIT](https://opensource.org/licenses/MIT)
    *   Project: [https://github.com/dmotz/trystero](https://github.com/dmotz/trystero)
*   **QRCode.js**
    *   License: [MIT](https://opensource.org/licenses/MIT)
    *   Project: [https://davidshimjs.github.io/qrcodejs/](https://davidshimjs.github.io/qrcodejs/)
*   **SunCalc**
    *   License: BSD-2-Clause
    *   Project: [https://github.com/mourner/suncalc](https://github.com/mourner/suncalc)
*   **Google Generative AI SDK**
    *   License: [Apache 2.0](https://www.apache.org/licenses/LICENSE-2.0)
    *   Project: [https://github.com/google/generative-ai-js](https://github.com/google/generative-ai-js)
*   **@imgly/background-removal**
    *   License: AGPL v3
    *   Project: [https://github.com/imgly/background-removal-js](https://github.com/imgly/background-removal-js)

## Fonts & Typography
*   **Google Fonts**
    *   Included Fonts: Inter, Bricolage Grotesque, DynaPuff, Domine, Climate Crisis, JetBrains Mono, DotGothic16, Playpen Sans, Jaro, Doto, Nunito, Material Symbols Rounded.
    *   License: [SIL Open Font License (OFL)](https://scripts.sil.org/cms/scripts/page.php?site_id=nrsi&id=OFL)
*   **Open Runde**
    *   A rounded variant of the Inter font family.
    *   License: [SIL Open Font License (OFL)](https://scripts.sil.org/cms/scripts/page.php?site_id=nrsi&id=OFL)
*   **Inter Numeric**
    *   A rounded, variable numeric variant of the Inter font family.
    *   License: [SIL Open Font License (OFL)](https://scripts.sil.org/cms/scripts/page.php?site_id=nrsi&id=OFL)

## Assets & Media
Note: Wallpaper information is seperate from this file.
*   **Setup Music**
    *   Track: "Brittle Rille - Reunited" by Kevin MacLeod
    *   License: [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/)
    *   Source: [incompetech.com](http://incompetech.com)
*   **UI Sounds**
    *   Various interface sound effects.
    *   Source: [Notification Sounds](https://notificationsounds.com)
    *   License: [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/)
*   **Timer Sound**
    *   Source: Google
*   **Fallback Fanny**
    *   Character "Fanny" courtesy of jacknjellify (BFDI)
*   **Polygol 16 Branding**
    *   Character "Sixteen" courtesy of jacknjellify (BFDI)
*   **Airy OOBE**
    *   Character "Airy" courtesy of Cheesy Hfj (ONE)

## Data & Services
*   **Weather Data**
    *   Provided by [Open-Meteo.com](https://open-meteo.com/)
    *   License: [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/)
*   **Geocoding Data**
    *   Provided by [OpenStreetMap](https://www.openstreetmap.org/copyright) (Nominatim)
    *   License: ODbL (Open Database License)

---

Polygol source code is licensed under CC BY-NC 4.0 unless otherwise noted in specific submodules or third-party libraries listed above.
